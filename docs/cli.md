...
- **`reference-only`** — no local snapshot was produced; the entry keeps its source as a pointer. The `reason` is `not-fetched` (an in-repo `repo`+`path` whose repo has not been fetched yet), `no-source` (a non-http(s) URL), `fetch-error` (the fetch failed — surfaced as a loud `WARNING`), `unsupported-binary` (a binary format such as PDF — the text pipeline would destroy it, so the source is kept as a pointer and never rewritten; surfaced as a `NOTE`), or `snapshot-disabled` (the atlas entry declares `"snapshot": false`).

An atlas doc entry may declare `"snapshot": false` to opt out of snapshotting entirely — for a source whose fetch cannot capture the real content, such as a login-gated page that answers with the site's generic index. The entry keeps its canonical `url` (or `repo`+`path`) as a citable pointer, and any previously-written snapshot variants are removed on the next run, so a stale misleading snapshot cannot regenerate or survive the opt-out. As a backstop for sources not yet opted out, `fetch-docs` prints a `WARNING` when two url sources produce identical snapshot content — a strong login-wall / redirect-to-index signal.
